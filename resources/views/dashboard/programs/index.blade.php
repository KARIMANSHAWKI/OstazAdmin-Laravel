    ...
    <div class="layout-px-spacing">
        <div class="page-header">
            <div class="page-title">
                <h3 style="display: inline-block">Programs</h3>
                <a id="btn-compose-mail" class="btn-add "  data-toggle="modal" data-target="#addProgramModal"
                        ...
                        <tbody>
                            @foreach ($programs as $program)
                                <tr id="tr{{ $program->id }}">
                                    ...
                                    @if (isset($program->trainers))
                                    <td>
                                    {{count($program->trainers)}}
                                    </td>
                                    @endif
                                    <td>{{  getStatus($program->status) }}</td>
                                    <td>
                                        <ul class="table-controls" style="text-align: center">
                                            <li><a href="javascript:void(0);" onclick="editProgram({{$program->id}})"
                                            ...
                                            <li><a href="javascript:void(0);" data-toggle="modal" data-target="#deleteModal" onclick="getId({{$program->id}})"
        ...
        @include('dashboard.programs.edit')
        @include('dashboard.programs.delete')
        @include('dashboard.programs.create')
