    ...
    <div class="layout-px-spacing">
        ...
        <div class="page-header">
            <div class="page-title">
                <h3 style="display: inline-block">Supervisors </h3>
                <a id="btn-compose-mail" class="btn-add "  data-toggle="modal" data-target="#addSupervisorModal"
                    ...
                    <table id="range-search" class="display table" style="width:100%">
                        ...
                        <tbody>
                            @foreach ($supervisors as $supervisor)
                                <tr id="tr{{ $supervisor->id }}">
                                    <td>{{ $supervisor->first_name .' '. $supervisor->last_name }}</td>
                                    <td>{{ $supervisor->email }}</td>
                                    <td>{{ getStatus($supervisor->status) }}</td>
                                    <td>
                                        <ul class="table-controls" style="text-align: center">
                                            <li><a href="javascript:void(0);" onclick="editSupervisor({{ $supervisor->id }})"
                                            ...
                                            <li><a href="javascript:void(0);" data-toggle="modal" data-target="#deleteModal" onclick="getSupervisorId({{$supervisor->id }})"
        ...
        @include('dashboard.supervisors.delete')
        @include('dashboard.supervisors.create')
