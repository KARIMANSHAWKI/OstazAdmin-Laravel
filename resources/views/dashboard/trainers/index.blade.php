    ...
    <div class="layout-px-spacing">
        ...
        <div class="page-header">
            <div class="page-title">
                <h3 style="display: inline-block">Trainers </h3>
                <a style="width: 200px" id="btn-compose-mail" class="btn-add "  data-toggle="modal" data-target="#addTrainerModal"
                        ...
                        <tbody>
                            @foreach ($trainers as $trainer)
                                <tr id="tr{{ $trainer->id }}">
                                    ...
                                    <td>{{ $trainer->email }}</td>
                                    <td>{{ $trainer->age }}</td>
                                    <td>{{ getGender($trainer->gender) }}</td>
                                    <td>{{ $trainer->country->name }}</td>
                                    <td>{{ $trainer->phone }}</td>
                                    <td>{{ getStatus($trainer->status) }}</td>
                                    <td>
                                        <ul class="table-controls">
                                            <li><a href="javascript:void(0);" onclick="editTrainer({{ $trainer->id }})"
                                            ...
                                            <li><a href="javascript:void(0);" data-toggle="modal" data-target="#deleteModal" onclick="getTrainerId({{$trainer->id }})"
        ...
        @include('dashboard.trainers.edit')
        @include('dashboard.trainers.delete')
        @include('dashboard.trainers.create')
